...
18. **Backup/restore is manual, file-based**: an Export button serializes all IndexedDB stores to a downloadable JSON file; an Import button (`<input type="file">`) reads one back in and restores. Deliberately plain browser APIs (download/upload), not File System Access — works uniformly on desktop and mobile. This is also the "manual cloud backup" mechanism for now: user drops the exported JSON into Drive/Dropbox/etc. by hand.
    - **Revisited (2026-09-01)**: added an *optional* auto-backup layer on top, not a replacement — Settings → "Link backup file…" uses the File System Access API (`showSaveFilePicker`) to get a persistent handle to a file (suggested name `ext-trkr.json`), stored in a new `meta` IndexedDB store (schema bumped to v2). Every mutating db.js call (transactions, accounts, tags) then silently rewrites that file via `syncLinkedBackupFile()`. This deliberately reintroduces the File System Access API that #17 rejected — accepted knowingly as desktop-Chromium-only, degrading gracefully to "unsupported, use manual Export" everywhere else (feature-detected in Settings). The manual Export/Import in this item is unchanged and still the cross-platform path.
...
- Optional auto-backup (2026-09-01, see §3 item 18's revisit note): Settings → "Link backup file…" (desktop Chromium only, File System Access API) links a persistent handle to `ext-trkr.json`; `db.js`'s `syncLinkedBackupFile()` then silently rewrites it after every mutating call (transactions, accounts, tags). Settings shows link status and offers re-authorization after permission lapses (e.g. browser restart) and unlinking. Manual Export/Import unchanged.
