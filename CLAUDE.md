...
18. **Backup/restore is manual, file-based**: an Export button serializes all IndexedDB stores to a downloadable JSON file; an Import button (`<input type="file">`) reads one back in and restores. Deliberately plain browser APIs (download/upload), not File System Access — works uniformly on desktop and mobile. This is also the "manual cloud backup" mechanism for now: user drops the exported JSON into Drive/Dropbox/etc. by hand.
    - **Revisited then reverted (2026-09-01 → 2026-09-06)**: briefly added an *optional* auto-backup layer (Settings → "Link backup file…", File System Access API, `showSaveFilePicker`, schema bumped to v2 for a `meta` store). Reverted once the real primary use case became clear: **phone as the primary device** (see §4 item 21/22 and the 2026-09-06 collaboration note) — the File System Access API doesn't exist on mobile at all, so the feature was dead weight for the one device that actually matters, working only in the desktop-Chromium fallback nobody was using it from. The `meta` object store is harmlessly left in the v2 schema rather than migrating existing installs down to remove it. Manual Export/Import (this item, unchanged) remains the only backup mechanism, and is the cross-platform path by design.
...
- `sw.js`'s cached app-shell file list had fallen out of sync with `js/` (missing `filters.js`/`recurring.js` added later) — offline mode would fail to load them after install. Fixed by updating the list and bumping `CACHE_NAME` so already-installed clients pick up the fix.
- Export button reported "Backup downloaded" but nothing appeared on-device (reported from the installed mobile PWA) — `exportToFile()` was calling `URL.revokeObjectURL()` immediately after `a.click()`, racing the browser's own (not always synchronous, especially on mobile) handling of the download and silently breaking it with no error to catch. Fixed by delaying the revoke.
...
- Account names in the Analysis summary section are now clickable links that navigate to the Transactions tab, filtered by that account and the current analysis time period.
- Optional auto-backup-to-linked-file was added then reverted — see §3 item 18's note. Not shipped.
